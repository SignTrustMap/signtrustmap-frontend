...
type DraftImage = SurveyImage & { submissionId: string; sessionId?: string; chunkUploaded?: boolean; uploadCompleted?: boolean };
...
export function useSaveSurveyDraft() {
  ...
  return async (image: SurveyImage, request: CreateSubmissionDto, submissionId?: string) => {
    if (!session) throw new Error('Sign in to save a draft.');
    if (submissionId && attempt.current?.submissionId !== submissionId) {
      attempt.current = await readDraftImage(session.account.id, submissionId) ?? { ...image, submissionId };
    }
    if (!attempt.current || attempt.current.uri !== image.uri) {
      ...
      attempt.current = { ...image, submissionId: submissionId ?? readSubmissionId(await create.mutateAsync({ request })) };
    }
