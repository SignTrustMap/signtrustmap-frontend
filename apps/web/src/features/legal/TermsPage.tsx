...
export default function TermsPage() {
  ...
  const sections = useMemo(
    () => (t('terms.sections', { returnObjects: true }) as LegalSection[]) || [],
    [t]
  )
  ...
  useEffect(() => {
    function handleScroll() {
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        let current = sections[0]?.id || 'scope'
          ...
          <aside className="lg:col-span-4 lg:sticky lg:top-[120px] self-start z-20">
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                <FileText size={16} className={isDark ? 'text-[#00c4de]' : 'text-[#007b8b]'} />
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-xl text-xs font-medium transition-colors duration-150 flex items-center justify-between cursor-pointer ${
                        isActive
                          ? isDark
                            ? 'bg-[#00c4de]/15 text-[#00c4de] font-bold border border-[#00c4de]/30'
                            : 'bg-teal-50 text-[#007b8b] font-bold border border-teal-200'
                          : isDark
              ...
              >
                ...
                <span>{t('terms.alert_disclaimer')}</span>
              </div>
            </div>
          </aside>
          ...
          <div className="lg:col-span-8 space-y-6">
                ...
                <div className="flex items-center gap-2.5 mb-3">
                  <span
                    className={`font-mono text-xs font-bold px-2 py-0.5 rounded-lg border ${
                      isDark
                        ? 'bg-white/5 border-white/10 text-[#00c4de]'
                        : 'bg-gray-100 border-gray-200 text-[#007b8b]'
                    }`}
                  >
                    {sec.number}
                  </span>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span>{t('terms.copyright', { year: new Date().getFullYear() })}</span>
              </div>
              <Link
                to="/privacy"
